    ...
    return (
        <div className='cont-list-packages' >
            <ul className='ul-list-packages' >
                {props.packages.map((item) => <li key={item + '-item'} className='element-package' ><button className='text-gray-500 dark:text-white' onClick={() => { props.selectPackage(item); }} >{item}</button></li>)}
            </ul>
