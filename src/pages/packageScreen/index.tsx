...
const PackageScreen: FunctionComponent = () => {
  ...
  function filterPackage(search_term: string) {
    let result = packageList.filter(
      (item) => item.toLowerCase().indexOf(search_term) !== -1,
    );
    ...
    getListPackets(
      ...
      (_error) => {
        setCommands((previewState: CommandI[]) => [
          ...previewState,
          {
            str: `Command list package error: "${_error}"`,
  ...
  return (
    <div className="">
      ...
      <ListPackages
        haveError={haveError}
        errorText={errorText}
        packages={packageListFilter}
        selectPackage={(str) => {
          setPackageActive(str);
          navigate("/commands");
        }}
      />
    </div>
