...
export function getTypeAndModelDevice(str: string): Device | null {
  ...
  result = {
    id: str.substring(0, positionFirstWhiteSpace),
    name: str.substring(startPosition + "product:".length, lastPosition),
    type: TypeOfDeviceEnum.ANDROID,
  };

  return result;
}

export function getTypeAndModelDeviceIOS(
  data: IosDeviceFromSimctlJson,
): Device {
  return {
    id: data.udid,
    name: data.name,
